    ...
    >
        ...
        >
          ...
          >
            ...
            >
              ...
              >
                <div
                  class="text-dark-gray-dark-gray-2 text-center font-['Roboto-SemiBold',_sans-serif] text-sm font-semibold relative flex items-center justify-center"
                >
                  Copy Link
                </div>
              </div>
      ...
      >
        ...
        >
          ...
          >
            <div
              v-for="(scan, index) in analytics.recentScans"
              :key="index"
...
export default {
    ...
    return {
      analytics: {
        ...
        recentScans: [
          ...
          {
            id: 3,
            timestamp: "2025-07-23 12:20:08",
            userAgent: "Mozilla/5.0 (Linux; Android 12; SM-G998B) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/92.0.4515.159 Mobile Safari/537.36",
            browser: "Chrome 92.0",
  ...
  computed: {
    totalScans() {
      return this.analytics.recentScans.length;
    },
  },
  mounted() {
    // Update analytics based on the passed qrItem
    if (this.qrItem) {
      this.analytics.qrType = this.qrItem.type;
      this.analytics.created = this.qrItem.date;
    }
  },
  methods: {
    closePopup() {
    ...
    copyLink() {
      if (this.qrItem && this.qrItem.url) {
        navigator.clipboard.writeText(this.qrItem.url).then(() => {
      ...
      if (this.qrItem && this.qrItem.url) {
        ...
        this.$nextTick(() => {
          ...
          if (qrCanvas) {
            ...
            link.download = `qr-code-${this.qrItem.type || 'download'}.png`;
    ...
    shareQR() {
      if (this.qrItem && this.qrItem.url) {
        if (navigator.share) {
          ...
          navigator.share({
            title: `QR Code - ${this.qrItem.type || 'Website'}`,
            text: `Check out this QR code for ${this.qrItem.type || 'Website'}`,
            url: this.qrItem.url
          }).then(() => {
    ...
    fallbackShare() {
      ...
      const url = encodeURIComponent(this.qrItem.url);
      const text = encodeURIComponent(`Check out this QR code for ${this.qrItem.type || 'Website'}`);
      ...
      const shareOptions = [
        ...
        { name: 'Email', url: `mailto:?subject=QR Code&body=${text} ${this.qrItem.url}` }
      ];
