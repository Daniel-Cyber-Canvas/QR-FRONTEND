<template>
    <div class="min-h-screen bg-gray-50">
                ...
                <div class="bg-[#ffffff] p-2.5 flex flex-col gap-3 items-start justify-start self-stretch flex-1 relative overflow-hidden">
                    ...
                    <div class="flex flex-col gap-6 items-start justify-start flex-1 relative max-w-none w-full">
                        ...
                        <div class="flex flex-col gap-6 items-start justify-start self-stretch shrink-0 relative">
                            <div class="flex flex-col gap-6 items-start justify-start self-stretch shrink-0 relative">
                                <form @submit.prevent="generateAppQRCode" class="w-full">
                                        ...
                                        <div class="flex flex-col gap-3 items-start justify-start self-stretch shrink-0 relative">
                                            ...
                                            <div class="px-3.5 pb-3 flex flex-row gap-[18px] items-start justify-end self-stretch shrink-0">
                                                ...
                                                >
                                                    ...
                                                    <span v-else>Generate App QR Code</span>
                                                </button>
                                            </div>
                                        </div>
                                    </div>
                        ...
                        <div class="flex flex-col gap-4 items-start justify-start w-full shrink-0 relative">
                            <div class="bg-white rounded border-solid border-[#e2e8f0] border p-4 flex flex-col gap-4 items-start justify-start w-full relative">
                                ...
                                <div v-else-if="!isLoading && qrItems.length === 0" class="flex flex-col items-center justify-center py-8 text-gray-500">
                                    <Icon name="ph:device-mobile" class="w-12 h-12 mb-2" />
                                    <p>No dynamic app QR codes found</p>
        ...
        <DeleteConfirmationPopup
            ...
            :item-type="selectedQRItem.analytics?.type || 'App QR'"
...
export default {
    ...
    methods: {
        async generateAppQRCode() {
            ...
            try {
                const title = this.formData.app_name.trim() || 'AppQR';
                ...
                console.log('🚀 Sending app payload:', payload);
                ...
                if (response.data) {
                    ...
                    // Check if redirect_url is actually a backend URL (not the app URL)
                    ...
                    if (isBackendRedirectUrl) {
                        ...
                        console.warn('⚠️ redirect_url contains app URL instead of backend URL:', response.data.redirect_url);
                        console.error('❌ Backend configuration issue - redirect_url should point to backend, not app URL');
                    ...
                    console.log('✅ Generated App QR with redirect URL:', qrContent);
                } else {
                ...
                alert(`Failed to generate App QR code: ${errorMessage}`);
            } finally {
        ...
        async fetchQRCodes() {
            this.isLoading = true;
            try {
                console.log('🔍 Starting fetchQRCodes for apps...');
                ...
                if (response.data && response.data.data) {
                    ...
                    // Filter for app service
                    console.log('🔍 Filtering for app service...');
                    qrData = qrData.filter(item => item.service === 'app');
                    console.log('🔍 Filtered app QRs:', qrData.length);

                    console.log('📊 Processing', qrData.length, 'app QR codes');

                    this.qrItems = qrData.map(item => {
                        console.log('🔍 Processing QR item:', item);
                        
                        // Determine display name and URL
                        let displayName = 'App QR';
                        let url = '';
                        ...
                        if (item.content) {
                            if (typeof item.content === 'string') {
                                try {
                                    ...
                                    displayName = parsedContent.name || item.title || 'App QR';
                                    url = parsedContent.url || '';
                                } catch (e) {
                                    displayName = item.title || 'App QR';
                                    url = item.content;
                                }
                            } else if (typeof item.content === 'object') {
                                displayName = item.content.name || item.title || 'App QR';
                        ...
                        console.log('✅ Processed app QR item:', processedItem);
                        return processedItem;
                    });

                    console.log('✅ Final processed app QR items:', this.qrItems);
                } else {
                    console.log('📭 No app QR codes found');
                    this.qrItems = [];
                }
            } catch (error) {
                console.error('❌ Error fetching app QR codes:', error);
        ...
        async fetchQRCodesSilent() {
            ...
            try {
                console.log('🔍 Starting silent fetchQRCodes for apps...');
        ...
        handleEdit(qrItem) {
            console.log('🔧 Edit clicked for App QR Item:', qrItem);
            try {
                if (qrItem) {
        ...
        handleDownload(qrItem) {
            console.log('📥 Download clicked for App QR Item:', qrItem);
            try {
                if (qrItem) {
        ...
        handleDelete(qrItem) {
            console.log('🗑️ Delete clicked for App QR Item:', qrItem);
            try {
                if (qrItem) {
                    this.selectedQRItem = { ...qrItem };
                    this.$nextTick(() => {
        ...
        handleAnalytics(qrItem) {
            console.log('📊 Analytics clicked for App QR Item:', qrItem);
            try {
                if (qrItem) {
                    this.selectedQRItem = qrItem;
        ...
        async saveEditedQRCode(updatedData) {
            try {
                console.log('💾 Saving edited App QR code:', updatedData);
                ...
                // Handle the updated app content
                ...
                const payload = {
                    type: 'dynamic',
                    service: 'app',
                    title: contentToSave.name || 'AppQR',
                ...
                if (response.data) {
                    console.log('✅ App QR code updated successfully');
                    this.closeEditPopup();
                    await this.fetchQRCodesSilent();
                    alert('App QR code updated successfully!');
                } else {
                    throw new Error('No response data received');
                }
            } catch (error) {
                console.error('❌ Error updating App QR code:', error);
                console.error('❌ Error response:', error.response?.data);
                const errorMessage = error.response?.data?.message || error.message || 'Unknown error';
                alert(`Failed to update App QR code: ${errorMessage}`);
            }
        },
        ...
        async confirmDelete() {
            ...
            try {
                ...
                console.log('🗑️ Deleting App QR code:', this.selectedQRItem.id);
                
                const response = await axios.delete(`/api/qr/${this.selectedQRItem.id}`);
                
                console.log('✅ App QR code deleted successfully');
                this.closeDeleteConfirmation();
                
                await this.fetchQRCodesSilent();
                alert('App QR code deleted successfully!');
            } catch (error) {
                console.error('❌ Error deleting App QR code:', error);
                const errorMessage = error.response?.data?.message || error.message || 'Unknown error';
                ...
                if (error.response?.status === 404) {
                    ...
                    alert(`Failed to delete App QR code: ${errorMessage}`);
                }
            } finally {
