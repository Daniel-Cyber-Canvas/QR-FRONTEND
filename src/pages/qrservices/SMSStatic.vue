...
<template>
    <div class="min-h-screen bg-gray-50">
                                        ...
                                        <div class="flex flex-col gap-3 items-start justify-start self-stretch shrink-0 relative">
                                            ...
                                            <div class="px-3.5 pb-3 flex flex-row gap-[18px] items-start justify-end self-stretch shrink-0">
                                                ...
                                                >
                                                    ...
                                                    <span v-else>Generate SMS QR Code</span>
                                                </button>
                                            </div>
                                        </div>
                                    </div>
                        ...
                        <div class="flex flex-col gap-4 items-start justify-start w-full shrink-0 relative">
                            <div class="bg-white rounded border-solid border-[#e2e8f0] border p-4 flex flex-col gap-4 items-start justify-start w-full relative">
                                ...
                                <div v-else-if="!isLoading && qrItems.length === 0" class="flex flex-col items-center justify-center py-8 text-gray-500">
                                    <Icon name="ph:chat-circle" class="w-12 h-12 mb-2" />
                                    <p>No static SMS QR codes found</p>
        ...
        <DeleteConfirmationPopup
            ...
            :item-type="selectedQRItem.analytics?.type || 'SMS QR'"
        ...
        async generateSMSQRCode() {
            ...
            try {
                const payload = {
                    type: "static",
                    service: "sms",
                    title: `SMS QR - ${this.formData.sms_phone}`,
                    content: {
                        service: "sms",
                ...
                console.log('🚀 Sending static SMS QR payload:', payload);

                const response = await axios.post('/api/qr', payload);
                
                if (response.data) {
                    // For static QR codes, create SMS URL
                    const qrContent = this.generateSMSURL(
                        this.formData.sms_phone.trim(),
                        this.formData.sms_message.trim()
                    );
        ...
        async fetchQRCodes() {
            ...
            try {
                ...
                if (response.data) {
                    ...
                    // Client-side filter for static SMS QRs
                    qrData = qrData.filter(qr => {
                        const isStaticSMSQR = qr.type === 'static' && 
                                             qr.content && 
                                             qr.content.service === 'sms';
                        return isStaticSMSQR;
                    });

                    console.log('📱 Filtered static SMS QRs:', qrData);

                    this.qrItems = qrData.map(qr => {
                        const smsUrl = this.generateSMSURL(
                            qr.content.phone || '',
                            qr.content.message || ''
                        );
                        ...
                        return {
                            ...
                            url: smsUrl,
                            qrCodeValue: smsUrl,
                            displayName: qr.title || `SMS QR - ${qr.content.phone}`,
                            selected: false,
